...
An Ocelloids network is a collection of nodes (section \ref{sec:node}) under a common trust zone with vetted access for participation.
...
An Ocelloids node is a logical entity that could be deployed in multiple physical systems, i.e. in horizontally scalable clusters.
Essentially, a logical network node provides:
\begin{enumerate}
  \item Ingress services for sourcing on-chain data.
